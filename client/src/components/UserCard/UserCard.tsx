...
import { changeUserInfo, changeUserInfoById } from '@/functions/userOperations'
...
const getCardMode = (user: TUser, updater: TUser) => {
  ...
  if (updater.role == UserRole.ADMIN || updater.role == UserRole.ROOT)
    res = CardMode.UPDATE_ADMIN
  return res
}
...
const UserCard: React.FC<UserCardProps> = ({ user, updater }) => {
  ...
  const adminUp = async () => await changeUserInfoById(user.id, updateUser)
  const selfUp = async () => await changeUserInfo(updateUser)
  useEffect(() => {
        ...
        <div className={styles.avatar_block}>
          ...
          {mode != CardMode.VIEW && (
            <CardButton
              ...
              action={mode === CardMode.UPDATE_ADMIN ? adminUp : selfUp}
            />
          )}
        </div>
