...
export const TasksTable = () => {
	...
	const [appointedDoneTasks, setAppointedDoneTasks] = useState<ITask[] | null>(
		null
	) // CLIENT
	const [appointedNotDoneTasks, setAppointedNotDoneTasks] = useState<
		ITask[] | null
	>(null)

	const [executedDoneTasks, setExecutedDoneTasks] = useState<ITask[] | null>(
		null
	) // EXECUTOR
	const [executedNotDoneTasks, setExecutedNotDoneTasks] = useState<
		ITask[] | null
	>(null)
	...
	const getAdminAndRoot = async (userRole: string) => {
		...
		tasksDb?.map(task => {
			if (
				...
				task.status != TaskStatus.VERIFYREJECTED &&
		...
		try {
			...
			if (viewingUser?.role != UserRole.CLIENT) {
				...
				if (viewingUser?.role != UserRole.EXECUTOR) {
					await getAdminAndRoot(
						viewingUser?.role ? viewingUser?.role : UserRole.ADMIN
					)
				}
			}
		} catch {
		...
		<div className={styles.tasksTable}>
			<div className={styles.container}>
				...
				{(viewingUser?.role == UserRole.EXECUTOR ||
					...
					viewingUser?.role == UserRole.ADMIN) && (
					<>
						{executedNotDoneTasks != null &&
							executedNotDoneTasks.length != 0 && (
								<div className={styles.table}>
									<span className={styles.title}>Выполняемые вами задачи</span>
									<TasksContainer
										tasks={executedNotDoneTasks}
